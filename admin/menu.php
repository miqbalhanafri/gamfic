...
$tampilan = query("SELECT updates.user, SUM(DISTINCT points) AS total, COUNT(updates.levels) AS jmlreview, user.username, uploadfoto.gambar, uploadfoto.motivasi, MAX(DISTINCT updates.levels) AS levelmu
	...
	ORDER BY total DESC, jmlreview DESC LIMIT 10
	");
...
<div class="w3-container w3-margin-top w3-large w3-mobile">
	...
	<div class="w3-row">
	  <div class="w3-col w3-container w3-xxlarge"><h1>Leaderboard Klasemen Web Design</h1></div>
	</div>
	...
	, welcome to Gamfic (SRL and Gamification) administrator. </p>
...
<!-- Looping for the div table -->
	...
	<div class="w3-row w3-theme-l1">	
	  ...
	  <div class="w3-col w3-container w3-half"><h1 style="color: yellow;">Ranking Gamfic</h1></div>
...
<!-- End looping for the div table -->
		...
		<?php foreach ( $tampilan as $row ) : ?>
			  ...
			  <div class="w3-col w3-container w3-quarter"><a href="#" class="w3-button w3-section w3-theme-l1 w3-ripple w3-round-large" style="width:100%;"><h1><i class='fas fa-dollar-sign'></i> <?= $row["total"]; ?></h1><p><?= $row["jmlreview"]; ?> Reviews</p></a></div> 
			</div>
