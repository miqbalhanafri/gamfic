...
$reflectedit = query("SELECT * FROM reflect WHERE id=$id")[0]; 
// var_dump($mhs[0]["nama"]);
...
if ( isset($_POST["submit"]) ) {
  ...
  if ( ubahreflectgamfic($_POST) >= 0 ) {
    ...
    echo "
      ...
      document.location.href = 'two_reflect_edit.php';
      </script>
    ";
  }
...
<div class="w3-container w3-margin-top w3-mobile">
	...
	<h1>Edit refleksi belajar level</h1>

	<p>Silakan anda modifikasi atau edit refleksi belajar anda.</p>
	<p>
		<textarea class="textinput w3-round-xxlarge" rows="20" style="margin-top: 15px; width: 100%; border:none; padding: 10px;" name="reflection" required><?= $reflectedit["reflection"]; ?></textarea>
		<input type="hidden" name="user" value="<?= $reflectedit["user"]; ?>">
		<input type="hidden" name="id" value="<?= $reflectedit["id"]; ?>">
	</p>

	<p>Catatan : Silakan anda isi setelah melakukan pembelajaran pada setiap level.</p>

	<p>
	<button type="submit" name="submit" class="w3-button w3-section w3-theme-d2 w3-ripple w3-round-xxlarge"> Save</button> <a href="two_reflect.php" class="w3-button w3-section w3-theme-d2 w3-ripple w3-round-xxlarge"><i class='fas fa-undo'></i></a>
	</p>
